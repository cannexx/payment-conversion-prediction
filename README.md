...
Streamlit Community Cloud 무료 플랜에서는 일정 시간 트래픽이 없으면 앱이 슬립 모드로 전환됩니다. 이를 막으려고 핑(ping) 장치를 두 가지 겹쳐 운영합니다.

1. **GitHub Actions** (`.github/workflows/keep-alive.yml`): 10분 간격 cron으로 앱에 요청. 다만 GitHub의 scheduled workflow는 저장소 활동이 적으면 실행이 밀리거나 건너뛸 수 있어 보조 수단으로만 씁니다.
